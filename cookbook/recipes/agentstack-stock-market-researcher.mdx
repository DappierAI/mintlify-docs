...
agentstack g t company_overview \
  --description="As of {timestamp}, fetch the company overview for {company_name} using real-time web search. Include company profile, industry, sector, CEO, headquarters location, employee count, market capitalization and stock ticker symbol." \
...
agentstack g t financials_performance \
  --description="As of {timestamp}, use real-time web search to extract financial performance data for {company_name}, including Revenue (TTM), Net Income (TTM), YoY revenue growth, gross margin, and recent quarterly trends." \
...
agentstack g t competitive_benchmarking \
  --description="As of {timestamp}, identify 3–5 peer companies in the same sector as {company_name}. Compare key metrics: P/E ratio, revenue, stock price, and market cap. Highlight metrics where {company_name} outperforms or underperforms." \
...
agentstack g t real_time_stock_snapshot \
  --description="As of {timestamp}, convert {company_name} to its ticker and retrieve a real-time stock snapshot using Dappier. Include current price, % daily change, volume, 52W high/low, P/E, EPS, dividend yield, and chart data for 1D, 5D, 1M, YTD, 1Y." \
...
agentstack g t news_and_sentiment \
  --description="As of {timestamp}, convert {company_name} to ticker and fetch real-time financial news using Dappier. Categorize news by: Earnings, Analyst Ratings, Market Moves, Partnerships, Legal/Regulatory." \
  --expected_output="Categorized list of real-time financial news headlines for {company_name} organized by topic." \
  --agent=stock_insights_analyst
```
