---
title: "🧱 Build a Sports News Summarizer Using Replit and Dappier AI Recommendations API"
---
...
<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/-2Nm6XKI8vs?si=KXS9pYLAeDihmJgs"
...
- **data_model_id (str)**: The **Sports News Model ID** from the **Dappier Marketplace**.
- **similarity_top_k (int)**: The number of articles to return (default: `9`).
- **ref (str)**: The domain to prioritize (e.g., `"sportsnaut.com"`).
- **num_articles_ref (int)**: The number of guaranteed articles from the chosen domain.
- **search_algorithm (str)**: `"semantic"` for contextual matching or `"most_recent"` for sorting by latest articles.
...
4. Add a new secret:
   - **Key:** `DAPPIER_API_KEY`
   - **Value:** _(Paste your Dappier API Key here)_
5. Click **Save**.
...
4. Add a new secret:
   - **Key:** `OPENAI_API_KEY`
   - **Value:** _(Paste your OpenAI API Key here)_
5. Click **Save**.
