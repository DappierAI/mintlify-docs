...
In this video, see how to connect your LLM to Dappier data models via Anthropic's MCP, an open standard that enables developers to build secure, two-way connections between their data sources and AI-powered tools. In this example, connect your AI to stock market data from the world's leading markets and exchanges to fetch real-time pricing and ticker data.

<iframe
    width="560"
    height="315"
    src="https://www.youtube.com/embed/Fow1bUBb3zI?si=vz99Jp3v1Ol6AKyI"
...
👉 [Download Claude for Desktop](https://claude.ai/download) (_Supports macOS & Windows_)

After installation:

-   Ensure you have the **latest version** → Click **Claude Menu** → **“Check for Updates…”**
-   **Linux** is _not_ yet supported.

<Accordion title="Why Use Claude for Desktop Instead of Claude.ai?">
    MCP currently supports **local desktop servers** for security and
    performance reasons. **Remote hosting** is in active development.
</Accordion>
...
<Accordion title="Hammer icon missing / server not showing in Claude">
    1. **Restart Claude for Desktop** 2. **Check `claude_desktop_config.json`
    for errors** 3. **Ensure the API Key is correctly entered** 4. **Verify your
    internet connection** 5. **Try running the server manually**: ```bash uvx
    dappier-mcp ```
</Accordion>
...
<Accordion title="Dappier tools not responding">
    If Claude detects the server but fails to retrieve results: 1. **Check
    Claude logs for errors** 2. **Restart Claude Desktop** 3. **Ensure your
    Dappier API Key is valid** 4. **Try manually running the server**: ```bash
    uvx dappier-mcp ```
</Accordion>
