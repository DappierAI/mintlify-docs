...
The **Dappier Sales Agent MCP** is a remote Model Context Protocol (MCP) server that lets any AI agent or LLM client **discover Dappier advertising inventory, launch Sponsored Conversations campaigns with a conversational Brand Agent creative, manage them, and pull delivery metrics** — all without writing HTTP wrappers around the Dappier REST API.

It implements the [Advertising Context Protocol (AdCP)](https://adcontextprotocol.org/) **3.1** sales-agent surface for Dappier and exposes **8 AI-callable tools** on the `media_buy` protocol.
...
<Note>
A visual walkthrough is **coming soon**.
</Note>
